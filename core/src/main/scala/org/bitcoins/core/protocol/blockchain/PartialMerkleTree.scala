...
object PartialMerkleTree {
  ...
  private def reconstruct(numTransaction: Int, hashes: Seq[DoubleSha256Digest], bits: scodec.bits.BitVector): BinaryTree[DoubleSha256Digest] = {
    ...
    def loop(remainingHashes: Seq[DoubleSha256Digest], remainingMatches: scodec.bits.BitVector, height: Int, pos: Int): (BinaryTree[DoubleSha256Digest], Seq[DoubleSha256Digest], scodec.bits.BitVector) = {
      if (height == maxHeight) {
